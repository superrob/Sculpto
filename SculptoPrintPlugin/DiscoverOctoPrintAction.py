...
class DiscoverOctoPrintAction(MachineAction):
    ...
    def startDiscovery(self):
        if not self._network_plugin:
            self._network_plugin = Application.getInstance().getOutputDeviceManager().getOutputDevicePlugin("SculptoPrintPlugin")
    ...
    def getStoredKey(self):
        ...
        if global_container_stack:
            ...
            if "sculptoprint_id" in meta_data:
                return global_container_stack.getMetaDataEntry("sculptoprint_id")
    ...
    def setApiKey(self, api_key):
        ...
        if global_container_stack:
            if "Sculptoprint_api_key" in global_container_stack.getMetaData():
                global_container_stack.setMetaDataEntry("sculptoprint_api_key", api_key)
            else:
                global_container_stack.addMetaDataEntry("sculptoprint_api_key", api_key)
    ...
    def apiKey(self):
        global_container_stack = Application.getInstance().getGlobalContainerStack()
        if global_container_stack:
            return global_container_stack.getMetaDataEntry("sculptoprint_api_key")
        else:
            return ""
    ...
    def _createAdditionalComponentsView(self):
        ...
        path = QUrl.fromLocalFile(os.path.join(PluginRegistry.getInstance().getPluginPath("SculptoPrintPlugin"), "SculptoPrintPlugin.qml"))
